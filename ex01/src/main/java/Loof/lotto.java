...
public class lotto {
	...
	public static void main(String[] args) {
		 ...
		 * 4. while(true) 사용해서 중복없는 랜덤 정수값 배열에 저장
		 * 5. for 문을 사용해서 두배열 일치 값 개수를 확인하고 등수를 출력
		 * 6. 참고 사항 - 일반적으로 이중 for문을 사용해 간편히 랜덤 로또 번호를 저장할 수 있으나,
		 *             while 문을 사용해서 작성
		 */
		
		//로또 당첨 정답 번호 배열입니다 (총 6개 정답번호가 저장)
		int lotto[] = {3, 5, 24, 32, 10, 9};
		System.out.println("로또 당첨 번호 : "+Arrays.toString(lotto));
		
		//랜덤 정수값을 생성하기 위해 Random 객체를 생성
		Random random = new Random();
		
		//random 랜던 정수값을 담을 배열을 선언해줍니다
		int number[] = new int[6];
		Arrays.fill(number, 0); //배열에 초기 전체 데이터 0을 모두 대입
		
		int idx = 0; //배열 번지 번호를 지정하기 위한 인덱스 변수값 생성
		while(true) { 
			int zero = 0; //0이 아닌값이 정상적으로 모두 저장된것인지 확인 위해 변수 선언
			for(int i=0; i<number.length; i++) {
				if(number[i] == 0) { //배열 각 번지를 전체 확인 하면서 0 값이 있을 경우 zero 값을 증가
					zero ++;
				}
			}
			//zero 변수값이 카운트된게 없을 경우 정상적으로 값이 모두 대입된 것을 확인
			if(zero <= 0) {				
				break; //while 무한 루프를 탈출		
			}			
			else { //배열에 랜덤 정수값을 다 저장하지 않은 경우 수행
				//랜덤 정수값 1부터 45범위까지 임의 정수를 저장
				...
				for(int k=0; k<number.length; k++) {
					if(number[k] == random_number) {
						check ++; //배열 각 번지를 전체 확인 하면서 일치하는 정수값이 있을 경우 check 값을 증가
					}
				}
